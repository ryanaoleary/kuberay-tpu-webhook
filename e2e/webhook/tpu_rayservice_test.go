...
func waitForRayServiceReady(t *testing.T, serviceName string, timeout time.Duration) {
	...
	err := wait.PollUntilContextTimeout(t.Context(), 10*time.Second, timeout, true, func(ctx context.Context) (bool, error) {
		unstructSvc, err := dynamicClient.Resource(rayServiceGVR).Namespace(testNamespace).Get(ctx, serviceName, v1.GetOptions{})
		if err != nil {
			return false, err
		}
...
func verifyRayServiceCompletions(t *testing.T, serviceName string) {
	...
	curlCmd := []string{
		...
		"-d", `{"model": "Qwen/Qwen1.5-0.5B-Chat", "prompt": "San Francisco is a", "max_tokens": 15, "temperature": 0}`,
	}
...
func waitForRayServiceHeadPod(t *testing.T, serviceName string) string {
	...
	err := wait.PollUntilContextTimeout(t.Context(), 1*time.Second, 120*time.Second, true, func(ctx context.Context) (bool, error) {
